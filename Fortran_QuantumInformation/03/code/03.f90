...
program performance
    ...
    ! get command-line arguments (enable debugging)
    ...
    ! check if command line argument -debug for debugging is enabled
    ! if yes, enable debug and stop at errors/warnings
    ...
    ! ask user to insert the dimensions of the two matrices to multiply
    print *, "-----------------------------------------------------------------"
    print *, "Program: matrix matrix multiplication performance"
    print *, "Author:  Rocco Ardino"
    print *, "If you want to enable debugging functionalities, run:"
    print *, "./executable_name -debug"
    print *, "-----------------------------------------------------------------"
    print *, NEW_LINE('a')
    print *, "Now, insert the dimensions of the matrices to multiply."
    print *, "Insert mat1 dimensions:"
    read(*,*) N, M
    print *, "Insert mat2 dimensions:"
    read(*,*) P, Q


    ! Pre-condition: check if the first matrix is not too big in dimensions
    call check(debug     = DEBUG, &
               condition = (N>10000).OR.(M>10000), &
               msg_type  = "Warning", &
               msg       = "mat1 size too big", &
               trg_stop  = TRIGGER_STOP)
    ! Pre-condition: check if the second matrix is not too big in dimensions
    call check(debug    = DEBUG, &
               condition= (P>10000).OR.(Q>10000), &
               msg_type = "Warning", &
               msg      = "mat2 size too big", &
               trg_stop = TRIGGER_STOP)
